...
const Hero = ({content,isLoading}: {content?: HeroType, isLoading?: boolean}) => {
	...
	return (
		<div className={`w-full flex justify-start items-start h-[480px] pb-16 md:pb-24 relative`}>
			...
			<div className={"max-w-full overflow-hidden w-full min-h-96 relative md:static mx-auto py-5 flex flex-col gap-3 md:gap-5 items-center z-[1]"}>
				<Image src={"/vectors/transparent-track.svg"} alt={'track'} width={1200} height={800} className={'object-contain object-bottom absolute -translate-y-1/2 top-0 max-w-none -z-[1]'} />
				{withLinks && (
					<Carousel
						opts={{
							align: "start",
					...
					>
						<CarouselContent className={'gap-3 px-4'}>
								{content.links.map((l, index, arr) => (
									<CarouselItem key={index} className={`pl-0 basis-1/2 sm:basis-1/3 lg:basis-1/6 ${!!arr.length ? `lg:basis-1/${arr.length}` : "" }`}>
				...
				<ParseContent text={content.title} mainTitleClassName={'font-extrabold mt-12 text-white text-4xl md:text-5xl lg:text-6xl 2xl:text-7xl mb-4 uppercase text-center'} secondaryTitleClassName={"border-b-2 border-b-secondary w-fit font-bold text-lg text-white lg:text-2xl capitalize"} />
