...
const FederationCard = ({federation}: {federation: Federation}) => {
	return (
		<div className={'rounded-xl bg-white shadow-md h-fit'}>
			<Image src={getImageUrl(federation.logo.url)} alt={federation.logo.name} width={450} height={240} className={'w-full h-auto'} />
			<div className={'flex justify-between gap-3 items-end p-4'}>
				<div className={'flex gap-3 items-center flex-[3] h-[70px]'}>
					<Image src={getImageUrl(federation.region.flag.url)} alt={federation.region.name} width={70} height={70} className={'rounded-lg w-16 h-16 object-cover'} />
